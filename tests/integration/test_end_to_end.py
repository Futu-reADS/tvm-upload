...
def test_upload_marks_file_for_cleanup(mock_cw_boto3, mock_upload_boto3, test_config):
    ...
    mock_s3.head_object.side_effect = ClientError({"Error": {"Code": "NotFound"}}, "head_object")
    ...
    assert (
        str(test_file.resolve()) in system.disk_manager.uploaded_files
...
"""
    ...
    try:
        ...
        assert not test_file.exists(), "File should be immediately deleted when keep_days=0"
...
"""
    ...
    try:
        ...
        system2 = TVMUploadSystem(str(config_file))
        assert system2.queue_manager.get_queue_size() == 1, "Queue should persist across restart"
