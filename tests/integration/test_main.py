...
"""
    ...
    def system(self, temp_config_file, temp_log_dir):
        ...
        with (
            patch("src.upload_manager.boto3.session.Session") as mock_boto3_upload,
    ...
    """
        ...
        with (
            ...
            try:
                ...
                if expected_result:
                    assert result is True, f"Should allow upload at {now} (within 09:00-16:00)"
                else:
                    assert result is False, f"Should block upload at {now} (outside 09:00-16:00)"
    ...
    """
        ...
        with (
            ...
            try:
                ...
                assert (
                    result is False
                ), "Files should queue for scheduled upload when operational_hours disabled"
...
"""
        ...
        with (
            ...
            try:
                ...
                with patch.object(
                    system.upload_manager, "upload_file", side_effect=track_upload
                ) as mock_upload:
                    system.start()
                    time.sleep(0.5)
                    ...
                    assert (
                        len(upload_calls_during_start) == 0
                    ), "No files should be uploaded during start when upload_on_start=false"
    ...
    def test_disk_cleanup_after_batch_upload(self, system, temp_log_dir):
        ...
        with patch.object(system.config, "get", return_value=True):
            with patch.object(system.disk_manager, "get_disk_usage", return_value=(0.92, 1000, 80)):
                with patch.object(system.upload_manager, "upload_file", return_value=True):
                    with patch.object(
                        system.disk_manager, "cleanup_old_files", return_value=2
                    ) as mock_cleanup:
                        system._process_upload_queue()
