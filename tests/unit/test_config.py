...
def test_valid_deletion_config():
    ...
    config = {
        ...
        "disk": {"reserved_gb": 70},
        "deletion": {
            "after_upload": {"enabled": True, "keep_days": 14},
            "age_based": {"enabled": True, "max_age_days": 7, "schedule_time": "02:00"},
            "emergency": {"enabled": False},
        },
        "s3_lifecycle": {"retention_days": 14},
        "monitoring": {"cloudwatch_enabled": True, "metrics_publish_interval": 3600},
    }
...
def test_invalid_deletion_after_upload_enabled():
    ...
    config = {
        ...
        "upload": {"schedule": "15:00"},
        "disk": {"reserved_gb": 70},
        "deletion": {"after_upload": {"enabled": "yes"}},  # Should be boolean
    }
...
def test_invalid_deletion_keep_days_negative():
    ...
    config = {
        ...
        "upload": {"schedule": "15:00"},
        "disk": {"reserved_gb": 70},
        "deletion": {"after_upload": {"keep_days": -5}},  # Negative not allowed
    }
...
def test_invalid_scan_existing_files_max_age():
    ...
    config = {
        ...
        "upload": {
            "schedule": "15:00",
            "scan_existing_files": {"enabled": True, "max_age_days": -1},  # Negative not allowed
        },
        "disk": {"reserved_gb": 70},
    }
...
def test_invalid_s3_lifecycle_retention():
    ...
    config = {
        ...
        "upload": {"schedule": "15:00"},
        "disk": {"reserved_gb": 70},
        "s3_lifecycle": {"retention_days": 0},  # Must be > 0
    }
...
def test_log_directory_missing_path():
    ...
    config = {
        "vehicle_id": "vehicle-001",
        "log_directories": [{"source": "test"}],  # Missing 'path'
    ...
    try:
        with pytest.raises(
            ...
            match="(path.*required|missing.*path|Missing required field.*path)",
        ):
            ConfigManager(temp_path)
    finally:
        Path(temp_path).unlink()
...
def test_log_directory_missing_source():
    ...
    config = {
        "vehicle_id": "vehicle-001",
        "log_directories": [{"path": "/tmp/logs"}],  # Missing 'source'
        "s3": {"bucket": "test", "region": "us-east-1", "credentials_path": "/tmp/creds"},
        "upload": {"schedule": "15:00"},
        "disk": {"reserved_gb": 70},
    }
    ...
    try:
        with pytest.raises(
            ConfigValidationError,
            match="(source.*required|missing.*source|Missing required field.*source)",
        ):
            ConfigManager(temp_path)
    finally:
        Path(temp_path).unlink()
...
def test_log_directory_empty_path():
    ...
    config = {
        "vehicle_id": "vehicle-001",
        "log_directories": [{"path": "", "source": "test"}],
        "s3": {"bucket": "test", "region": "us-east-1", "credentials_path": "/tmp/creds"},
        "upload": {"schedule": "15:00"},
        "disk": {"reserved_gb": 70},
    }
...
def test_log_directory_empty_source():
    ...
    config = {
        "vehicle_id": "vehicle-001",
        "log_directories": [{"path": "/tmp/logs", "source": ""}],
        "s3": {"bucket": "test", "region": "us-east-1", "credentials_path": "/tmp/creds"},
        "upload": {"schedule": "15:00"},
        "disk": {"reserved_gb": 70},
    }
...
def test_log_directory_source_with_path_separator():
    ...
    config = {
        "vehicle_id": "vehicle-001",
        "log_directories": [{"path": "/tmp/logs", "source": "ros/logs"}],  # Invalid: contains /
        "s3": {"bucket": "test", "region": "us-east-1", "credentials_path": "/tmp/creds"},
        "upload": {"schedule": "15:00"},
        "disk": {"reserved_gb": 70},
    }
    ...
    try:
        with pytest.raises(
            ConfigValidationError,
            match="(source.*separator|invalid.*source|Must contain only|underscores)",
        ):
            ConfigManager(temp_path)
    finally:
        Path(temp_path).unlink()
...
def test_recursive_defaults_to_true():
    ...
    config = {
        "vehicle_id": "vehicle-001",
        "log_directories": [{"path": "/tmp/logs", "source": "test"}],  # No recursive field
        "s3": {"bucket": "test", "region": "us-east-1", "credentials_path": "/tmp/creds"},
        "upload": {"schedule": "15:00"},
        "disk": {"reserved_gb": 70},
    }
...
def test_recursive_boolean_true():
    ...
    config = {
        "vehicle_id": "vehicle-001",
        "log_directories": [{"path": "/tmp/logs", "source": "test", "recursive": True}],
        "s3": {"bucket": "test", "region": "us-east-1", "credentials_path": "/tmp/creds"},
        "upload": {"schedule": "15:00"},
        "disk": {"reserved_gb": 70},
    }
...
def test_recursive_boolean_false():
    ...
    config = {
        "vehicle_id": "vehicle-001",
        "log_directories": [{"path": "/tmp/logs", "source": "test", "recursive": False}],
        "s3": {"bucket": "test", "region": "us-east-1", "credentials_path": "/tmp/creds"},
        "upload": {"schedule": "15:00"},
        "disk": {"reserved_gb": 70},
    }
...
def test_recursive_integer_value_rejected():
    ...
    config = {
        "vehicle_id": "vehicle-001",
        "log_directories": [{"path": "/tmp/logs", "source": "test", "recursive": 1}],
        "s3": {"bucket": "test", "region": "us-east-1", "credentials_path": "/tmp/creds"},
        "upload": {"schedule": "15:00"},
        "disk": {"reserved_gb": 70},
    }
...
def test_pattern_valid_wildcard():
    ...
    for pattern in patterns:
        config = {
            "vehicle_id": "vehicle-001",
            "log_directories": [{"path": "/tmp/logs", "source": "test", "pattern": pattern}],
            "s3": {"bucket": "test", "region": "us-east-1", "credentials_path": "/tmp/creds"},
            "upload": {"schedule": "15:00"},
            "disk": {"reserved_gb": 70},
        }
...
def test_pattern_empty_string_rejected():
    ...
    config = {
        "vehicle_id": "vehicle-001",
        "log_directories": [{"path": "/tmp/logs", "source": "test", "pattern": ""}],
        "s3": {"bucket": "test", "region": "us-east-1", "credentials_path": "/tmp/creds"},
        "upload": {"schedule": "15:00"},
        "disk": {"reserved_gb": 70},
    }
...
def test_pattern_non_string_rejected():
    ...
    config = {
        "vehicle_id": "vehicle-001",
        "log_directories": [{"path": "/tmp/logs", "source": "test", "pattern": 123}],
        "s3": {"bucket": "test", "region": "us-east-1", "credentials_path": "/tmp/creds"},
        "upload": {"schedule": "15:00"},
        "disk": {"reserved_gb": 70},
    }
...
def test_pattern_with_path_separator_accepted():
    ...
    config = {
        "vehicle_id": "vehicle-001",
        "log_directories": [{"path": "/tmp/logs", "source": "test", "pattern": "*/subdir/*.log"}],
        "s3": {"bucket": "test", "region": "us-east-1", "credentials_path": "/tmp/creds"},
        "upload": {"schedule": "15:00"},
        "disk": {"reserved_gb": 70},
    }
...
def test_pattern_optional():
    ...
    config = {
        "vehicle_id": "vehicle-001",
        "log_directories": [{"path": "/tmp/logs", "source": "test"}],  # No pattern
        "s3": {"bucket": "test", "region": "us-east-1", "credentials_path": "/tmp/creds"},
        "upload": {"schedule": "15:00"},
        "disk": {"reserved_gb": 70},
    }
...
def test_minimal_config_has_all_required_defaults():
    ...
    config = {
        ...
        "s3": {"bucket": "test-bucket", "region": "us-east-1", "credentials_path": "/tmp/creds"},
        "upload": {"schedule": "15:00"},
        "disk": {"reserved_gb": 70},
    }
...
def test_allow_deletion_valid_true():
    ...
    config = {
        "vehicle_id": "vehicle-001",
        "log_directories": [
            {"path": "/home/user/logs", "source": "terminal", "allow_deletion": True}
        ],
        "s3": {"bucket": "test", "region": "cn-north-1", "credentials_path": "~/.aws"},
        "upload": {"schedule": {"mode": "daily", "daily_time": "15:00"}, "file_stable_seconds": 60},
        "disk": {"reserved_gb": 70},
...
def test_allow_deletion_valid_false():
    ...
    config = {
        "vehicle_id": "vehicle-001",
        "log_directories": [{"path": "/var/log", "source": "syslog", "allow_deletion": False}],
        "s3": {"bucket": "test", "region": "cn-north-1", "credentials_path": "~/.aws"},
        "upload": {"schedule": {"mode": "daily", "daily_time": "15:00"}, "file_stable_seconds": 60},
        "disk": {"reserved_gb": 70},
    }
