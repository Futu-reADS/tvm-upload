...
class CloudWatchManager:
    ...
    def __init__(
        self, region: str, vehicle_id: str, enabled: bool = True, profile_name: str = None
    ):
            ...
            try:
                ...
                if endpoint_url:
                    ...
                    if profile_name:
                        ...
                        logger.info(
                            f"CloudWatch initialized with profile '{profile_name}' for region: {region}"
                        )
                    else:
                    ...
                    try:
                        ...
                        logger.error("=" * 60)
                        raise RuntimeError(
                            f"CloudWatch enabled but cannot publish metrics: {perm_error}"
                        )
                ...
                logger.error(
                    "  - If monitoring is optional, set monitoring.cloudwatch_enabled: false"
                )
                logger.error("=" * 60)
        ...
        try:
            ...
            if metrics:
                ...
                if days_accumulated > 1:
                    logger.warning("=" * 60)
                    logger.warning(
                        f"⚠️  ACCUMULATED METRICS: Publishing data for {days_accumulated} days"
                    )
                ...
                self.cw_client.put_metric_data(Namespace=CLOUDWATCH_NAMESPACE, MetricData=metrics)
...
if __name__ == "__main__":
    ...
    logger.info(
        f"Bytes: {cw.bytes_uploaded}, Files: {cw.files_uploaded}, Failures: {cw.files_failed}"
    )
